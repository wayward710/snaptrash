...
L.Control.Velocity = L.Control.extend({
	...
	options: {
		...
		// Could be 'm/s' for meter per second, 'km/h' for kilometer per hour, 'kt' for knots of 'Bft' for Beaufort
		speedUnit: 'm/s'
	},
	...
	vectorToSpeed: function vectorToSpeed(uMs, vMs, unit) {
		...
		// Default is m/s
		if (unit === 'km/h') {
			return this.meterSec2kilometerHour(velocityAbs).toFixed(2);
		} else if (unit === 'kt') {
			return this.meterSec2Knots(velocityAbs).toFixed(2);
		} else if (unit === 'Bft') {
			return this.meterSec2BFT(velocityAbs).toFixed(0);
		} else {
			return velocityAbs.toFixed(2);
		}
	},
	...
	meterSec2BFT: function meterSec2BFT(meters) {
		var result;
		switch (true) {
		case (meters < 0.514):
		result = 0;
	...
	_onMouseMove: function _onMouseMove(e) {
		...
		if (gridValue && !isNaN(gridValue[0]) && !isNaN(gridValue[1]) && gridValue[2]) {
			htmlOut = date.toLocaleString() + " <strong>" + this.options.velocityType + " Direction: </strong>" + self.vectorToDegrees(gridValue[0], gridValue[1], this.options.angleConvention).toFixed(2) + "°" + ", <strong>" + this.options.velocityType + " Speed: </strong>" + self.vectorToSpeed(gridValue[0], gridValue[1], this.options.speedUnit) + " " + this.options.speedUnit + "<br>";
		} else {
...
var Windy = function Windy(params) {
	...
	var createBuilder = function createBuilder(data) {
		...
		data.forEach(function (record) {
			uComp.data = record.variables.ugrd10m.data;
			vComp.data = record.variables.vgrd10m.data;
		});
	...
	var buildGrid = function buildGrid(data, callback) {
		...
		λ0 = record.variables.lon.attributes.minimum;
		φ0 = record.variables.lat.attributes.maximum; // the grid's origin (e.g., 0.0E, 90.0N)

		Δλ = record.variables.lon.attributes.resolution;
		Δφ = record.variables.lat.attributes.resolution; // distance between grid points (e.g., 2.5 deg lon, 2.5 deg lat)

		ni = parseInt(record.variables.lon.attributes.grads_size);
		nj = parseInt(record.variables.lat.attributes.grads_size); // number of grid points W-E and N-S (e.g., 144 x 73)

//		date = new Date("1970-01-01 00:00:00 UTC");     //time format for modern browsers
		date = new Date(Date.UTC(1970, 0, 1, 0, 0, 0)); //time format for old IE
		var cutoff = record.variables.time.data%1;
		switch (true) { //correct (partly wrong) rounded time values
		case (cutoff >= 0.09 && cutoff <0.15):
		record.variables.time.data = record.variables.time.data-cutoff+0.125;
		break;
		case (cutoff >= 0.19 && cutoff <0.35):
		record.variables.time.data = record.variables.time.data-cutoff+0.25;
		break;
		case (cutoff >= 0.35 && cutoff <0.45):
		record.variables.time.data = record.variables.time.data-cutoff+0.375;
		break;
		case (cutoff >= 0.59 && cutoff <0.7):
		record.variables.time.data = record.variables.time.data-cutoff+0.625;
		break;
		case (cutoff >= 0.75 && cutoff <0.85):
		record.variables.time.data = record.variables.time.data-cutoff+0.75;
		break;
		case (cutoff >= 0.85 && cutoff <0.95):
		record.variables.time.data = record.variables.time.data-cutoff+0.875;
		break;
		default:;
		break;
		}
		date.setHours(date.getHours() + (record.variables.time.data*24 - 719164*24 + 1));
		window.date = date;
		});

		// Scan mode 2 assumed. Longitude increases from λ0, and latitude increases from φ0.
